...
Future<void> run(HookContext context) async {
  ...
  await _generateStates(
      context, featureName, handlers, blocDir, isBloc, acronyms);

  if (isBloc) {
    await _generateEvents(context, featureName, handlers, blocDir, acronyms);
  }

  if (isBloc) {
    await _generateBloc(context, featureName, handlers, blocDir, acronyms);
  } else {
    await _generateCubit(context, featureName, handlers, blocDir, acronyms);
  }
}
...
Future<void> _generateStates(
  ...
  final fileName = '${toSnakeCaseWithAcronyms(featureName, acronyms)}_state';
...
Future<void> _generateEvents(
  ...
  final buffer = StringBuffer();
  final pascalName = featureName.pascalCase;
  final fileName = '${toSnakeCaseWithAcronyms(featureName, acronyms)}_event';
