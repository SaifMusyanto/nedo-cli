...
Future<void> run(HookContext context) async {
  if (!context.vars.containsKey('feature_name')) {
    context.vars['feature_name'] = context.logger.prompt(
  ...
  if (!context.vars.containsKey('type')) {
    context.vars['type'] = context.logger.chooseOne(
  ...
  if (!isBloc) {
    ...
    while (addingProps) {
      if (!context.logger
          .confirm('Add a property to the state?', defaultValue: true)) {
        addingProps = false;
        break;
      }

      final propName =
          context.logger.prompt('Property name (e.g. user, items):');
      final propType =
          context.logger.prompt('Property type (e.g. User?, List<Item>):');
      final propDefault = context.logger.prompt(
    ...
    if (!context.vars.containsKey('main_data_type')) {
      if (context.logger.confirm('Define a main data type for Success state?',
        ...
        context.vars['main_data_type'] =
            context.logger.prompt('Enter type (e.g. List<User>):');
      } else {
        context.vars['main_data_type'] = '';
      }
    }
  ...
  if (!context.vars.containsKey('methods')) {
    context.logger.info('No methods configuration found from previous steps.');
  ...
  final dependencies = <String, dynamic>{
    'hasSecureStorage': context.logger.confirm(
  ...
  for (final method in methods) {
    ...
    if (isBloc) {
      final concurrency = context.logger.chooseOne(
      ...
      if (concurrency != 'concurrent (default)') {
        ...
        imports.add("import 'package:bloc_concurrency/bloc_concurrency.dart';");
      }
    }

    final statePattern = context.logger.chooseOne(
